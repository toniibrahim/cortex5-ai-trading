...
void InitFromSizes(int inS,int h1S,int h2S,int h3S,int outS){ 
    ...
    // Where heads/output take input from
    int final_in = (has_lstm ? lstm_size : h3);

    if(has_dueling){
        // Dueling: Value is a scalar, Advantage is per action
        value_head.in  = final_in;
        value_head.out = 1;                 // <-- FIX: value head is 1, not value_head_size
        advantage_head.in  = final_in;
        ...
        // Non-dueling: single output layer
        SetupLayer(L4, final_in, outS);     // <-- FIX: use final_in (lstm_size if LSTM enabled)
    }

    if(has_lstm){
        lstm.Init(h3, lstm_size);           // <-- FIX: trainer uses LSTM(h3 → lstm_size)
    }
}
...
// Loads Double-Dueling DRQN model file with v4/v5 header compatibility (confidence head aware).
// ---- Helper: peek next Dense layer dims as 32-bit integers (no consumption) ----
bool PeekNextLayerDims(const int h, int &in_units, int &out_units)
{
   long pos = (long)FileTell(h);                // explicit cast to avoid ulong→long warning
   if(pos < 0) return false;

   if(FileIsEnding(h)) { FileSeek(h, pos, SEEK_SET); return false; }

   // Layer headers in your .dat store dims as 4-byte ints, not 8-byte longs:
   int inI  = (int)FileReadInteger(h, INT_VALUE);   // 4 bytes
   if(FileIsEnding(h)) { FileSeek(h, pos, SEEK_SET); return false; }
   int outI = (int)FileReadInteger(h, INT_VALUE);   // 4 bytes

   // Rewind — we only peek
   FileSeek(h, pos, SEEK_SET);

   // Sanity guard
   if(inI <= 0 || outI <= 0 || inI > 20000 || outI > 20000) return false;
...
// ===== Replace your LoadModel with this version =====
...
bool LoadModel(const string filename){
  ...
  if(magic==(long)0xC0DE0203 || magic==(long)0xC0DE0204) has_checkpoint=true;
  else if(magic==(long)0xC0DE0202) has_checkpoint=false;
  else { PrintFormat("EA LoadModel: unsupported magic 0x%I64X",magic); FileClose(h); return false; }

  // Symbol + TF (length-prefixed in v203+)
  ulong pos=FileTell(h);
  int sym_len=(int)FileReadLong(h);
  if(sym_len>0 && sym_len<=64){
    g_model_symbol=FileReadString(h,sym_len);
    g_model_tf=(ENUM_TIMEFRAMES)(int)FileReadLong(h);
  } else {
    FileSeek(h,(long)pos,SEEK_SET);
  ...
  g_h1=(int)FileReadLong(h);
  g_h2=(int)FileReadLong(h);
  g_h3=(int)FileReadLong(h);

  // ---- ARCH FLAGS & SIZES (consume all 8 longs written by trainer) ----
  bool file_has_conf=false;        // UseConfidenceOutput
  int  conf_head_size=0;           // ConfidenceHeadSize (payload dims later)
  int  seq_len_ignored=0;

  if(has_checkpoint){
    g_Q.has_lstm        = (FileReadLong(h)==1);   // UseLSTM
    g_Q.has_dueling     = (FileReadLong(h)==1);   // UseDueling
    file_has_conf       = (FileReadLong(h)==1);   // UseConfidenceOutput  <-- MISSING BEFORE

    g_Q.lstm_size       = (int)FileReadLong(h);   // LSTMSize
    seq_len_ignored     = (int)FileReadLong(h);   // SequenceLen (EA doesn't use)
    g_Q.value_head_size = (int)FileReadLong(h);   // ValueHeadSize (trainer still saves it)
    g_Q.advantage_head_size=(int)FileReadLong(h); // AdvHeadSize
    conf_head_size      = (int)FileReadLong(h);   // ConfidenceHeadSize   <-- MISSING BEFORE
  } else {
    g_Q.has_lstm=false; g_Q.has_dueling=false; g_Q.lstm_size=0;
    g_Q.value_head_size=0; g_Q.advantage_head_size=0;
  }

  // Verify STATE/ACTIONS
  if(stsz!=STATE_SIZE || acts!=ACTIONS){
    PrintFormat("EA LoadModel: shape mismatch (STATE/ACTIONS). File has %d/%d; EA expects %d/%d",
                stsz,acts,(int)STATE_SIZE,(int)ACTIONS);
    FileClose(h); return false;
  }

  // Build network scaffold (InitFromSizes now sets Value head → 1 and L4 input correctly)
  ...
  for(int i=0;i<STATE_SIZE;++i){ g_feat_min[i]=FileReadDouble(h); g_feat_max[i]=FileReadDouble(h); }

  // Checkpoint block (informative)
  if(has_checkpoint){
    ...
    Print("EA LoadModel: Model contains checkpoint data (training timestamp: ",
          TimeToString(last_trained,TIME_DATE|TIME_MINUTES), ", steps: ", training_steps, ")");
  }

  // ---- Load layers in trainer's order ----
  bool ok=true;
  ok = ok && LoadLayer(h,g_Q.L1);      // STATE_SIZE → h1
  ok = ok && LoadLayer(h,g_Q.L2);      // h1 → h2
  ok = ok && LoadLayer(h,g_Q.L3);      // h2 → h3
  if(!ok){ FileClose(h); return false; }

  if(g_Q.has_lstm){
    ok = ok && LoadLSTMLayer(h,g_Q.lstm); // h3 → lstm_size
    if(!ok){ FileClose(h); return false; }
  }

  if(g_Q.has_dueling){
    ok = ok && LoadLayer(h,g_Q.value_head);     // final_in → 1   (InitFromSizes FIX)
    ok = ok && LoadLayer(h,g_Q.advantage_head); // final_in → ACTIONS
  } else {
    ok = ok && LoadLayer(h,g_Q.L4);             // final_in → ACTIONS
  }
  if(!ok){ FileClose(h); return false; }

  // Optional Confidence head (skip payload cleanly if present)
  if(file_has_conf){
    int cin=(int)FileReadLong(h);
    int cout=(int)FileReadLong(h);
    if(cin<=0 || cout<=0 || cin>20000 || cout>20000){
      PrintFormat("EA LoadModel: invalid Confidence head dims %d×%d",cin,cout);
      FileClose(h); return false;
    }
    // Skip weights (cin*cout) + biases (cout), doubles (8 bytes each)
    long cur=(long)FileTell(h);
    long skip_bytes = 8 * ((long)cin * (long)cout + (long)cout);
    FileSeek(h, cur + skip_bytes, SEEK_SET);
  }
...
bool ScalePosition(double current_lots, double target_lots, bool is_long){
    ...
    if(volume_diff > 0){
        ...
        bool result = is_long ? 
            trade.Buy(add_volume, _Symbol, 0, 0, 0, "cortex5_scale_up") :
            trade.Sell(add_volume, _Symbol, 0, 0, 0, "cortex5_scale_up");
...
void MaybeTrade(const int action){
  ...
  if(InpEnforceSymbolTF && (_Symbol != g_model_symbol || _Period != g_model_tf)){
    ...
    if(TimeCurrent() - last_critical_alert > 60){ // Alert every minute for critical safety
      ...
      Alert("cortex5 CRITICAL: Trading blocked - model mismatch detected!");
  ...
  trade.SetExpertMagicNumber(InpMagic);
  bool ok = is_buy ? 
      trade.Buy(lots, _Symbol, 0, sl_price, tp_price, "cortex5_ai") :
      trade.Sell(lots, _Symbol, 0, sl_price, tp_price, "cortex5_ai");
...
int OnInit(){
  ...
  if(any_mismatch){
    ...
    if(InpEnforceSymbolTF){
      ...
      Print("================================================================================");
      Alert("cortex5 EA ERROR: Model mismatch detected. Check Expert log!");
      ...
      Alert("cortex5 EA WARNING: Trading with mismatched model! Performance will be poor!");
...
void OnDeinit(const int reason){
    ...
    Print("cortex5 EA shutting down. Reason: ", reason);
}
